...
class Merchi_Integration_Public
{
	...
	/**
	 * Retrieves the API URL and API key based on the current environment (staging or live).
	 *
	 * This method checks the plugin's configuration to determine whether it is operating in staging or live mode.
	 * It then returns the appropriate API endpoint URL and API secret key for making requests to the Merchi API.
	 *
	 * @return array An associative array containing:
	 *               - 'url' => string The base URL of the Merchi API (staging or live).
	 *               - 'key' => string The API secret key for authenticating requests.
	 */
	private function get_api_credentials() {
        $is_staging = MerchiSettings::get('merchi_staging_mode');
        if ($is_staging) {
            $api_url = MerchiSettings::get('staging_merchi_url');
            $api_key = MerchiSettings::get('staging_merchi_api_secret');
        } else {
            $api_url = MerchiSettings::get('merchi_url');
            $api_key = MerchiSettings::get('merchi_api_secret');
        }
